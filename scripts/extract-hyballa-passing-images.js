...
const PDF_PATH = process.argv[2]
  || '/Users/tor.inge.jossang@aftenbladet.no/Documents/Fotballbøker/Hyballa, Peter_ Poel, Hans-Dieter te - German Soccer Passing Drills _ More than 100 Drills from the Pros-Meyer & Meyer Sport (2015).pdf';
const DATA_PATH = path.join(ROOT, 'src/data/hyballa-exercises.ts');
const OUT_DIR = path.join(ROOT, 'public/book-illustrations/hyballa-passing');
const MANIFEST_PATH = path.join(ROOT, 'docs/books/illustrations/hyballa-passing-manifest.json');
const MAP_TS_PATH = path.join(ROOT, 'src/data/hyballa-image-map.ts');
...
function parseExercises() {
  ...
  const matches = [...src.matchAll(/\{[\s\S]*?id:\s*"([^"]+)"[\s\S]*?name:\s*"([^"]+)"[\s\S]*?source:\s*"hyballa"[\s\S]*?sourceRef:\s*"([^"]+)"[\s\S]*?\}/g)];
  return matches
    .map((match) => {
      const sourceRef = match[3];
      const figureMatch = sourceRef.match(/Fig\.\s*([0-9]+[a-z]?)/i);
      if (!figureMatch) return null;
      return {
        id: match[1],
        name: match[2],
        sourceRef,
        figure: figureMatch[1].toLowerCase(),
      };
    })
    .filter(Boolean);
}
...
function extractFigureLines(pageWords) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const directMatch = line.text.match(/\bfig\.?\s*([0-9]+[a-z]?)\b/i);
    if (directMatch) {
      figures.push({
        figure: directMatch[1].toLowerCase(),
        y: line.y,
        xMin: line.xMin,
        xMax: line.xMax,
        text: line.text,
    ...
    if (/^fig\.?$/i.test(line.text) && lines[index + 1] && /^\d+[a-z]?$/i.test(lines[index + 1].text)) {
      figures.push({
        figure: lines[index + 1].text.toLowerCase(),
        y: line.y,
...
function detectFigureBoxes(srcImage) {
  ...
  const roiW = imgW;
  const roiH = Math.round(imgH * 0.84);
...
function writeWebp(srcImage, crop, outPath) {
  ...
  run(`magick '${safeSrc}' -crop ${crop.w}x${crop.h}+${crop.x}+${crop.y} +repage ${trimArgs()} -strip -quality 78 '${safeOut}'`);
}
...
function main() {
  ...
  if (exercises.length === 0) throw new Error('Fant ingen Hyballa-øvelser med figurreferanse i datafilen');
  ...
  const tempDir = fs.mkdtempSync('/tmp/hyballa-passing-');
  ...
  for (const page of pages) {
    ...
    const pageItems = (pageEntries.get(page) || []).sort((a, b) => a.figure.localeCompare(b.figure));
    ...
    for (const entry of pageItems) {
      ...
      const imageUrl = `/book-illustrations/hyballa-passing/${fileName}`;
  ...
  const mapLines = manifestEntries
    ...
    .join('\n');
  const mapTs = `export const hyballaImageById: Record<string, string> = {\n${mapLines}\n};\n`;
